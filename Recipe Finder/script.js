...
async function searchRecipes() {
  ...
  try {
    ...
    if (data.meals) {
      ...
      recipesGrid.innerHTML =
        '<p style="text-align: center; grid-column: 1/-1;">No recipes found. Try another ingredient.</p>';
    }
  } catch (error) {
    recipesGrid.innerHTML =
      '<p style="text-align: center; grid-column: 1/-1;">Error fetching recipes. Please try again.</p>';
  }
...
async function showFavorites() {
  ...
  if (favoriteIds.length === 0) {
    recipesGrid.innerHTML =
      '<p class="error-message">No favorite recipes saved yet.</p>';
    loading.style.display = "none";
    return;
  }
...
async function searchRecipes() {
  ...
  try {
    ...
    showError(
      "Error fetching recipes. Please check your connection and try again."
    );
  }
